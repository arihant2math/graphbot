/*!
    This module contains various "context" definitions as bitflags.
    Context flags are defined as u64 values.
*/
...
// Basic (single-bit) flag list for debugging
const FLAGS: [(&str, u64); 38] = [
...
pub fn describe(context: u64) -> String {
    ...
    flags.sort_by_key(|&(_, value)| value);
    flags
        .into_iter()
        .map(|(name, _)| name)
        .collect::<Vec<&str>>()
...
mod tests {
    ...
    #[test]
    fn test_describe() {
        // Example: combining TEMPLATE_NAME, ARGUMENT_DEFAULT, and STYLE_BOLD.
        let context = TEMPLATE_NAME | ARGUMENT_DEFAULT | STYLE_BOLD;
        let desc = describe(context);
        // Expected order: TEMPLATE_NAME, ARGUMENT_DEFAULT, STYLE_BOLD.
        assert_eq!(desc, "TEMPLATE_NAME|ARGUMENT_DEFAULT|STYLE_BOLD");
    }
}
